...
import com.oru.iri.conf.Configuration.DefaultConfSettings;
import com.oru.iri.model.Hash;
import com.oru.iri.service.API;
...
/**
 * Main oru Reference Implementation starting class
 */
public class IRI {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        try {
            ...
            log.error("Exception during oru node initialisation: ", e);
            System.exit(-1);
        }
        log.info("oru Node initialised correctly.");
    }
    ...
    private static void validateParams(final Configuration configuration, final String[] args) throws IOException {
        ...
        if (args == null || (args.length < 2 && !configurationInit)) {
            log.error("Invalid arguments list. Provide ini-file 'oru.ini' or API port number (i.e. '-p 14600').");
            printUsage();
        }
    ...
    private static void shutdownHook() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            log.info("Shutting down oru node, please hold tight...");
            try {
                ixi.shutdown();
                api.shutDown();
                oru.shutdown();
            } catch (final Exception e) {
                log.error("Exception occurred shutting down oru node: ", e);
            }
        }, "Shutdown Hook"));
    }
}
